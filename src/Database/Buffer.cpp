...
void MiniBuffer::WriteBlockToDB(Database& db, int size)
{
    try
    {
        ...
        {
            auto data = ReadFromBuffer();

            // Database key is a pair of tick_n and drone_id
            std::pair<int, int> db_key = {std::stoi(data.tick_n), std::stoi(data.id)};
            if (ticks_and_ids.contains(db_key))
            {
                spdlog::warn("Drone {} at tick {} already written to the database", data.id, data.tick_n);
                continue;
            }
            else
            {
    ...
    while (true)
    {
        ...
        if (buffer_size > 0)
        {
            ...
            for (auto& data : data_vector)
            {
                ...
                auto tick_n = std::stoi(data.tick_n);
                if (mini_buffers.mini_buffers.contains(tick_n))
                {
                    // mini_buffers[data.tick_n].WriteToBuffer(data.data, data.check, data.tick_n);
                    mini_buffers.mini_buffers[tick_n]->WriteToBuffer(data);
                }
                else
                {
                    // MiniBuffer mini_bffr(data.tick_n);
                    // mini_buffers[index] = mini_bffr;
                    mini_buffers.mini_buffers[tick_n] = std::make_shared<MiniBuffer>(tick_n);
                    mini_buffers.mini_buffers[tick_n]->WriteToBuffer(data);
                }
            }
            ...
            if (buffer.getFaultsSize() > 0)
            {
                for (size_t _ = 0; _ < buffer.getFaultsSize(); ++_)
                {
                    ...
                    DroneData empty_data(
                        0, // tick_n
                        std::to_string(fault.drone_id), // id
                        "FAULT", // status
                        "-1.0", // charge
                        {fault.position.first, fault.position.second}, // position
                        std::to_string(fault.zone_id), // zone_id
                        "false" // checked
                    );
                    ...
                    if (reconnect)
                    {
                        empty_data.status = "RECONNECT";
                    }
                    else
                    {
                        empty_data.status = fault.fault_state;
                    }
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (mini_buffers_elements.first != mini_b->getID())
            {
                ...
                {
                    auto n_alive_drones = redis.hget("zone:" + std::to_string(i) + ":drones_alive_history",
                                                     std::to_string(mini_b->getID()));
            ...
            spdlog::info("Minibuffer {} needs {} n_elements, it has {} elements", mini_b->getID(),
                         mini_buffers_elements.second, mini_b->getSize());
            if (mini_b->getSize() >= mini_buffers_elements.second)
            {
                spdlog::info("Writing Minibuffer {} of size {} to db", mini_buffers.mini_buffers.begin()->first,
                             mini_b->getSize());

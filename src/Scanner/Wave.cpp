...
Wave::Wave(int tick_n, const int wave_id,
           Redis &shared_redis,
           TickSynchronizer &synchronizer) : tick(tick_n),
    ...
    // Old vector drones implementation
    // for (int i = 0; i < 300; i++) {
    ...
    for (int i = 0; i < 300; i++) {
        // Genera l'ID del drone
        int drone_id = id * 1000 + i;

        // Inserisci il drone nella mappa usando drone_id come chiave
        // drones.emplace(drone_id, Drone(drone_id, id, *this));

        Drone drone(drone_id, id, this);
        drones.insert({drone_id, drone});

        // Ora accedi al drone con drones[drone_id]
        drones[drone_id].position.x = 0;
        drones[drone_id].position.y = 0;
        drones[drone_id].starting_line.x = -2990.0f;
        drones[drone_id].starting_line.y = y;
        drones[drone_id].setState(ToStartingLine::getInstance());
        drones[drone_id].tick_drone = tick;

        // Calcola la direzione del drone
        float dx = -2990.0f;
        float dy = y;
        float distance = std::sqrt(dx * dx + dy * dy);
        drones[drone_id].dir.x = dx / distance;
        drones[drone_id].dir.y = dy / distance;

        // Incrementa y per il prossimo drone
        y += 20.0f;
    }
...
// void Wave::Move() {
//     X += 20;
//     for (auto &drone_pair : drones) {
//         auto &drone = drone_pair.second;
...
// controlla se sono stati se stati settati fault
// esegue run ∀ drone
// ∀ drone che ha espresso di esser distrutto, lo distrugge
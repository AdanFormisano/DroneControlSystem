...
namespace drone_control
{
    ...
    void DroneControl::ReadDataStream()
    {
        ...
        try
        {
            ...
            auto number_items_stream = redis.command<long long>("XLEN", "drone_data_dump");
            ...
            redis.xread("drone_data_dump", current_stream_id, std::chrono::milliseconds(10), number_items_stream,
                        std::inserter(new_result, new_result.end()));
            ...
            for (const auto& item : new_result)
            {
                ...
                for (const auto& stream_drone : item.second)
                {
                    ...
                    WorkOnStreamDump(stream_drone.second);
                }
    ...
    void DroneControl::WorkOnStreamDump(const std::vector<std::pair<std::string, std::string>>& data)
    {
        DroneData temp_data;
        ...
        buffer.WriteToBuffer(temp_data);
    }
    ...
    //     CheckDroneCharge(temp_drone_struct.data.id, temp_drone_struct.data.charge,
    //                      temp_drone_struct.data.charge_needed_to_base);
    //     CheckForSwap(temp_drone_struct.data.zone_id, temp_drone_struct.data.id, temp_drone_struct.data.charge,
    //                  temp_drone_struct.data.charge_needed_to_base);
    //
    ...
    void DroneControl::CheckDroneCharge(int drone_id, float current_charge, float charge_needed)
    {
        ...
        if ((redis.hget("drone:" + std::to_string(drone_id), "status") == "WORKING") &&
            (current_charge - (DRONE_CONSUMPTION_RATE * 80.0f) <= charge_needed) &&
    ...
    {
        auto cmd = redis.command<OptionalLongLong>("sismember", "zone:" + std::to_string(zone_id) + ":drones_active",
                                                   std::to_string(drone_id));
        int is_member = static_cast<int>(cmd.value_or(0));
        if (is_member &&
            (redis.get("zone:" + std::to_string(zone_id) + ":swap") != "started") &&
            (current_charge <= ((charge_needed * 2) + (40.0f * DRONE_CONSUMPTION_RATE))))
        {

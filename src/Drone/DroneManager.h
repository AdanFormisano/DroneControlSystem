...
namespace drones
{
    ...
    public:
        Redis& shared_redis;
        std::array<std::array<std::pair<float, float>, 4>, ZONE_NUMBER> zones_vertex;
        // Array of all the zones_vertex' vertex_coords_sqr, NOT global coords
    ...
    public:
        ...
        // Charge needed to go back to the base for every point in the drone_path
    ...
    private:
        ...
        float CalculateChargeNeededToBase(std::pair<float, float> furthest_point);
        // Calculate charge needed to go from furthest point to base

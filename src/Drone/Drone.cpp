...
namespace drones
{
    Drone::Drone(int id, DroneZone& drone_zone) : drone_id(id), dz(drone_zone), drone_redis(drone_zone.zone_redis)
    {
        ...
        drone_data = {
            ...
            {"status", utils::droneStateToString(drone_state)}, // FIXME: name of function
        ...
        else
        {
            ...
            swap_final_coords = {
                std::stof(drone_redis.hget(redis_id, "swap_final_x").value_or("0")),
    ...
    Drone::Drone(int id, DroneZone& drone_zone, drone_state_enum drone_state_ext) : drone_id(id), dz(drone_zone),
        drone_redis(drone_zone.zone_redis), drone_state(drone_state_ext)
    {
        spdlog::info("Drone {} created with state {}", drone_id, utils::droneStateToString(drone_state));
    ...
    void Drone::Run()
    {
        try
        {
            ...
            drone_redis.hincrby("zone:" + std::to_string(dz.getZoneId()) + ":drones_alive_history",
                                std::to_string(tick_n), 1);
            ...
            if (!connected_to_sys)
            {
                auto status = utils::droneStateToString(drone_state);
            ...
            case drone_state_enum::IDLE_IN_BASE:
                ...
                if (connected_to_sys)
                {
                    ...
                    if (cmd == "work")
                    {
                        ...
                        drone_data[1].second = utils::droneStateToString(drone_state_enum::TO_ZONE);
                    ...
                    {
                        // Remove the drone from the queue of zone drones
                        drone_data[1].second = utils::droneStateToString(drone_state_enum::TO_ZONE_FOLLOWING);
                        drone_redis.lpop("zone:" + std::to_string(dz.getZoneId()) + ":drones");
                        drone_redis.set("drone:" + std::to_string(drone_id) + ":command", "none");
            ...
            // Check if the drone is in range to start workingx
                ...
                {
                    drone_data[1].second = utils::droneStateToString(drone_state_enum::WORKING);
            ...
            case drone_state_enum::TO_ZONE_FOLLOWING:
                ...
                spdlog::info("TICK {}: Drone {} [{}%] is moving to zone (following) {} {}", tick_n, drone_id, drone_charge,
                             drone_position.first, drone_position.second);
#endif
                if (drone_position.first == swap_final_coords.first && drone_position.second == swap_final_coords.
                    second)
                {
                    // Arrived to the zone
                    drone_data[1].second = utils::droneStateToString(drone_state_enum::FOLLOWING);
            ...
            case drone_state_enum::WORKING:
                ...
                if (connected_to_sys)
                {
                    if (cmd == "charge")
                    {
                        drone_data[1].second = utils::droneStateToString(drone_state_enum::TO_BASE);
                        drone_redis.set("drone:" + std::to_string(drone_id) + ":command", "none");
            ...
            case drone_state_enum::TO_BASE:
                ...
                if (drone_position.first == 0 && drone_position.second == 0)
                {
                    ...
                    drone_data[1].second = utils::droneStateToString(drone_state_enum::WAITING_CHARGE);
            ...
            case drone_state_enum::WAITING_CHARGE:
                ...
                if (connected_to_sys)
                {
                    ...
                    drone_redis.srem("zone:" + std::to_string(dz.getZoneId()) + ":drones_alive",
                                     std::to_string(drone_id));
            ...
            case drone_state_enum::DEAD:
                ...
                drone_data[1].second = utils::droneStateToString(drone_state_enum::DEAD);
            ...
            // Create a drone fault in DZ
                dz.CreateDroneFault(drone_id, drone_state_enum::NOT_CONNECTED, drone_position, tick_n, tick_n + 20,
                                    reconnect_tick);

            // Set on redis the previous state
                last_state = utils::stringToDroneStateEnum(drone_data[1].second);
                drone_redis.hset(redis_id, "status", utils::droneStateToString(last_state));
            ...
            case drone_state_enum::FAULT_SWAP:
                    ...
                    second)
                {
                    ...
                    spdlog::warn("Drone {} is swapping with a faulty drone, setting state to {}", drone_id,
                                 utils::droneStateToString(fault_state));

                    // Set the drone to the last known state
                    drone_data[1].second = utils::droneStateToString(fault_state);
                    drone_redis.lpop("zone:" + std::to_string(dz.getZoneId()) + ":drones");
                    drone_redis.set("drone:" + std::to_string(drone_id) + ":command", "none");
                    drone_state = fault_state;
                    drone_redis.hset(redis_id, "status", utils::droneStateToString(fault_state));
    ...
    void Drone::SetChargeNeededToBase()
    {
        ...
        // Calculate the charge needed to go back to the base
        drone_charge_to_base = distance * DRONE_CONSUMPTION_RATE;
    ...
    {
        drone_charge -= DRONE_CONSUMPTION_RATE * move_distance;
    }

    void Drone::setDroneState(drone_state_enum state)
    {
        drone_state = state;
        drone_data[1].second = utils::droneStateToString(state);
    }
    ...
    drone_state_enum Drone::CheckDroneStateOnRedis()
    {
        ...
        if (!state.has_value() || state.value() == utils::droneStateToString(drone_state))
        {
